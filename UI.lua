local Break = {RainbowColorValue = 0, HUESelectionPosition = 0}
local PresetColor = Color3.fromRGB(255, 30, 40)
...
local BreakSkillLib = Instance.new("ScreenGui")
BreakSkillLib.Name = "BreakSkillLib"
BreakSkillLib.Parent = game.CoreGui
BreakSkillLib.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

coroutine.wrap(function ()
    while wait() do
        Break.RainbowColorValue = Break.RainbowColorValue + 1 / 255
        Break.HUESelectionPosition = Break.HUESelectionPosition + 1

        if Break.RainbowColorValue >= 1 then
            Break.RainbowColorValue = 0
        end
        if Break.HUESelectionPosition == 80 then
            Break.HUESelectionPosition = 0
        end
    end
end)()
...
function Break:CreateWindow(name, game, color, bind)
    CloseBind = bind or Enum.KeyCode.RightShift
    PresetColor = color or Color3.fromRGB(255, 30, 40)
    local fs = false
	...
	MainFrame.Parent = BreakSkillLib
    ...
    UserInputService.InputBegan:Connect(function (io, p)
        if io.KeyCode == CloseBind then
            if uitoggled == false then
                ...
                uitoggled = true
                wait(.5)
                BreakSkillLib.Enabled = false
            else
                MainFrame:TweenSize(UDim2.new(0, 706, 0, 484), Enum.EasingDirection.Out, Enum.EasingStyle.Quart, .6, true)
                BreakSkillLib.Enabled = true
                uitoggled = false
            end
        end
    end)
    function Break:CreateNot(desc, buttontitle)
